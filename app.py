...
def generate_predictions(_model, cat_maps, origin, destination, _hist_df):
    ...
    # Only predict for airlines actually observed on this route
    route_airlines = None
    route_stops = None
    dur_est = 240.0
    if _hist_df is not None and not _hist_df.empty:
        ...
        if not route.empty:
            if "airline" in route.columns:
                route_airlines = sorted(route["airline"].dropna().unique())
            if "stops" in route.columns:
                route_stops = sorted(route["stops"].dropna().unique().astype(int))
            if "flight_duration_minutes" in route.columns:
                med = route["flight_duration_minutes"].median()
                if pd.notna(med):
                    dur_est = med

    # Fallback to all airlines/stops if no historical data for this route
    airlines = route_airlines if route_airlines else sorted(cat_maps["airline"].keys())
    stops_options = route_stops if route_stops else [0, 1, 2]
...
st.caption(
    ...
    f"{df['airline'].nunique()} airlines, {df['stops'].nunique()} stop option(s), "
    f"and {df['departure_date'].nunique()} dates. "
    f"Only airlines observed on this route in historical data are shown."
)
...
# =========================================================================
st.markdown("---")
with st.expander(":gear: **Model & Data Information**", expanded=False):
...
| Parameter | Value |
|:--|:--|
| **Algorithm** | {model_type} (scikit-learn) |
...
""")
        ...
        n_airlines = len(cat_maps["airline"])
        n_origins = len(cat_maps["origin"])
        n_destinations = len(cat_maps["destination"])
        st.markdown(f"""
| Stat | Value |
|:--|:--|
| **Data Source** | Google Flights (SerpApi) |
| **Airlines Covered** | {n_airlines} |
| **Origin Airports** | {n_origins} ({', '.join(sorted(cat_maps['origin'].keys()))}) |
| **Destination Airports** | {n_destinations} ({', '.join(sorted(cat_maps['destination'].keys()))}) |
""")

    with info_right:
        st.markdown("#### Feature Importance")
        feat_names = [
            "Origin", "Destination", "Airline", "Stops",
            "Days to Departure", "Day of Week", "Month",
            "Weekend", "Holiday", "Flight Duration",
        ]
        ...
        fig_fi = go.Figure(go.Bar(
            ...
            orientation="h",
            marker_color=[
                "#00d4aa" if v >= fi_df["Importance"].quantile(0.75)
                else "#ffa726" if v >= fi_df["Importance"].quantile(0.25)
                else "#555"
    ...
    st.markdown(
        "*The model was trained on real Google Flights data collected via SerpApi. "
        "Predictions are filtered to airlines & stop options actually observed "
        "on the selected route, then generated via the trained gradient boosting ensemble.*"
    )
